...
app.post("/signUp", async (req, res) => {
    ...
    const createKeyResponse = await fetch("https://api.unkey.dev/v1/keys", requestOptions)
    const createKeyResponseJson = await createKeyResponse.json()

    if (createKeyResponseJson.error) return res.status(400).json({ error: createKeyResponseJson.error, keys: null })
